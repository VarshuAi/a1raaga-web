...
import { APK_SIZE, APK_URL as DEFAULT_APK_URL, GITHUB_PROFILE, VERSION_JSON_URL } from "@/lib/site";

const HEADLINE = [
  { text: "Pure sonic" },
  { text: "experience with", gradient: false },
  { text: "cinematic soul.", gradient: true },
];

const STATS = [
  { value: "320k", label: "Ultra HQ Audio" },
  { value: APK_SIZE.replace(" MB", ""), label: "MB APK Size" },
...
export function Hero() {
  ...
  const [tagline, setTagline] = useState("Story Creator Studio & 320 kbps Stream Downloader");
    ...
    fetch(VERSION_JSON_URL)
      ...
      .then((data) => {
        ...
        if (data.changelog) {
          ...
          if (lines.length > 0) {
            setTagline(lines[0].replace("-", "").replace("•", "").trim());
          }
        }
      })
      .catch(() => {});
  ...
  return (
    ...
    >
      ...
      <motion.div aria-hidden style={{ x: bgX, y: bgY }} className="absolute inset-0">
        <div className="absolute left-[8%] top-[12%] size-[320px] animate-blob rounded-full bg-accent/20 blur-[80px] md:size-[460px] md:blur-[130px]" />
        <div className="absolute right-[4%] top-[30%] size-[300px] animate-blob-slow rounded-full bg-accent-2/20 blur-[80px] md:size-[420px] md:blur-[130px]" />
        <div className="absolute bottom-[-6%] left-[30%] size-[280px] animate-blob rounded-full bg-accent-3/15 blur-[90px] [animation-delay:3s] md:size-[380px] md:blur-[140px]" />
      </motion.div>
        ...
        >
          <motion.div variants={badgeVariants}>
            <span className="inline-flex items-center gap-2 rounded-full border border-foreground/10 bg-foreground/[0.03] py-1.5 pl-1.5 pr-4 text-xs text-muted-foreground backdrop-blur">
              <span className="rounded-full bg-gradient-to-r from-accent to-accent-2 px-2.5 py-0.5 text-[10px] font-bold text-white shadow-[0_0_10px_var(--glow)]">
                OFFICIAL
              </span>
          ...
          <h1 className="pt-7 font-display text-[13.5vw] font-bold leading-[0.98] tracking-[-0.045em] sm:text-7xl lg:text-[5.4rem]">
            {HEADLINE.map((w) => (
          ...
          >
            A1 Swaara is a cinematic Android music streaming app featuring 320 kbps Ultra HQ playback, instant stream downloading, Story Creator Studio, and synchronized karaoke lyrics. Free forever — no ads, no account.
          </motion.p>
          ...
          <motion.div variants={badgeVariants} className="flex flex-wrap items-center gap-3 pt-9">
            <a href={apkUrl} target="_blank" rel="noopener noreferrer">
              <Button size="lg" className="group/btn shadow-[0_0_20px_var(--glow)]">
          ...
          >
            {STATS.map((s) => (
              <div key={s.label} className="border-l border-foreground/10 pl-3">
                ...
                <dd className="font-display text-xl font-bold sm:text-2xl">{s.value}</dd>
              </div>
            ))}
          </motion.dl>
        </motion.div>
        ...
        <div className="relative mx-auto hidden justify-center sm:flex" style={{ perspective: 1400 }}>
          ...
          >
            <span className="flex items-center gap-1.5">
              <Sparkles size={12} className="text-accent" /> Story Studio
              <span className="rounded-full bg-accent/20 px-1.5 py-px text-[8px] font-bold uppercase tracking-wider text-accent">New</span>
            </span>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, scale: 0.7 }}
      ...
      >
        Scroll to explore
        <ChevronsDown size={15} className="animate-bounce text-accent" />
      </motion.a>
    </section>
  );
}
